...
// 图形储存数据
const pointList: Array<{ id: string, name: string; data: any; show: boolean; color: string, height: number }> = []
// 当前选中的图形id
let pointId: string | null = null
export class editPointFunC {
    viewer: any;
    handler: any;
    isDrawPoint: boolean;
    pointList: [];
     ...
     */
    save = () => {
        const positions = this.viewer.entities.getById(pointId).point.hierarchy._callback().positions;
        for (let index in pointList) {
            if (pointList[index].id === pointId) {
                pointList[index] = form.value
                pointList[index].data = positions
            }
        }
        local.setJSON('point', JSON.stringify(pointList))
    }
    /**
    * 开启绘制
    */
