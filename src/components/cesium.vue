...
const initMap = async () => {
  ...
  handler.setInputAction(async function (movement: any) {
    ...
    if (pickedFeature) {
      ...
      var featuresPromise = await viewer.value.imageryLayers.pickImageryLayerFeatures(pickedFeature, viewer.value.scene);
...
const addWFSTileLayer = () => {
  ...
  // 发送请求获取WFS数据
  axios.get('geoserver/geoserver/tiger/ows', {
    params: wfsParams,
    headers: {
...
const screenHandleClick = () => {
  ...
  handler.setInputAction(async (event: any) => {
    ...
    var featuresPromise = await viewer.value.imageryLayers.pickImageryLayerFeatures(pickRay, viewer.value.scene);
    console.log("featuresPromise:", featuresPromise)
...
*/
function queryWFSByPoint(geoserverUrl: string, point: string, typeName: string, callback) {
