...
export const pointTogether = (viewer: Cesium.Viewer, data: any[], pixelRange: number = 25 ) => {
    ...
    // 添加监听函数
    customDataSource.clustering.clusterEvent.addEventListener(function (clusteredEntities, cluster) {
        // 关闭自带的显示聚合数量的标签
        ...
        if (clusteredEntities.length >= 50) {
            ...
            cluster.billboard.image = computeImgAndTxt(
                LocationOrange,
