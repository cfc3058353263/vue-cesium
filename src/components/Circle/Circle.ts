...
export class Circle {
    ...
    getDragPosition = (center: any, radius: any) => {
        ...
        const dlng =
            2 * Math.asin(Math.sin(distance / (2 * ellRadius)) / Math.cos((center.latitude * Math.PI) / 180));
    ...
    handlerLeftClick = () => {
        this.handler.setInputAction((event: any) => {
            ...
            if (this.isDrawCircle && !this.circle) {
                ...
                const dragPosition = this.getDragPosition(cartographic, pick.id.ellipse.semiMinorAxis._value);
                var longitude = Cesium.Math.toDegrees(cartographic.longitude);
                var latitude = Cesium.Math.toDegrees(cartographic.latitude);
                var height = cartographic.height;
                this.drawPoint(
                    Cesium.Cartesian3.fromDegrees(longitude + dragPosition.parseLng, latitude, height)
                );
