...
import { cartesian3_to_lng_lat, two_cartesian3_m, lng_lat_to_cartesian3 } from '@/components/utils/utils.ts'
...
export class Track {
            ...
            switch (this.perspective) {
                case 0:
                    if (center && orientation) {
                        ...
                        const newHpr:any = {heading: hpr.heading, pitch: Cesium.Math.toRadians(0), roll: Cesium.Math.toRadians(0)}
                        var newOrientation = Cesium.Transforms.headingPitchRollQuaternion(center,newHpr);
                        transform = Cesium.Matrix4.fromRotationTranslation(Cesium.Matrix3.fromQuaternion(newOrientation), center);
     ...
     * @returns 
     */
    createRoute = async (lngLatH:[], count: number = 30) => {
        // 连个坐标点之间生成差值的数量
        // cartesian3 保存数组
            ...
            if (index > 0) {
                for (let i = 0; i < count; ++i) {
                    ...
                    let cartesian3 = Cesium.Cartesian3.lerp(
                        Cesium.Cartesian3.fromDegrees(lngLatH[index - 1][0], lngLatH[index - 1][1], lngLatH[index - 1][2]),
